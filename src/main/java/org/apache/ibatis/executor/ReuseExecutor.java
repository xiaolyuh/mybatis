...
/**
 * ReuseExecutor，顾名思义，是可以重用的Executor。
 * 它重用的是Statement对象，它会在内部以sql语句为key，利用一个Map把创建的Statement都缓存起来，
 * 每次在执行一条SQL语句时，它都会去判断之前是否存在基于该SQL缓存的Statement对象，
 * 存在而且之前缓存的Statement对象对应的Connection还没有关闭的时候就继续用之前的Statement对象，
 * 否则将创建一个新的Statement对象，并将其缓存起来。因为每一个新的SqlSession都有一个新的Executor对象，
